...
def main():
    ...
    if tickers and quantities:
        try:
            ...
            quantities = [float(quantity) for quantity in quantities.split(',')]

            if len(tickers) != len(quantities):
                st.error("Le nombre de tickers et de quantités doit être identique.")
                return
            ...
            st.error("Veuillez vérifier que les tickers et quantités sont correctement formatés.")
